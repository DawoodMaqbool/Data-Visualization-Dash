...
app.layout = html.Div([
    ...
    html.Br(),
    html.Br(),
    html.H2("Information of all Accidents happened with respect to Alcohol in the selected time period", style={'textAlign': 'center'}),
    html.Br(),
    dcc.Graph(id='mymap')
])
...
def update_output(start_date, end_date):
    ...
    fig = px.scatter_mapbox(dff, lat='LATITUDE', lon='LONGITUDE', zoom=8, height=520, width=1365, template='plotly_dark', color='ALCOHOL_RELATED',center=dict(lat=-37.95557, lon=145.11385), mapbox_style="carto-positron", hover_data={'ACCIDENT_TYPE': True, 'ACCIDENT_TIME':True, 'LATITUDE': False, 'LONGITUDE': False, 'REGION_NAME': True, 'TOTAL_PERSONS': True, 'INJ_OR_FATAL': True, 'RMA': True})
